...
| # | 功能 | 状态 | 说明 |
|---|------|------|------|
| 1 | 💰 支付系统 | 部分完成 | 已补齐用户端基础 API + `/payment` 前端页面（固定套餐 + 自定义金额、支付宝/微信选择、金额计算弹窗、二维码展示 + 状态轮询）以及 `/payment/result` 结果页；后端已接入 ZPay/Stripe（可返回 `pay_url/qr_url`）并对回调做金额/币种基础校验；后续仍可继续补强更严格的幂等/审计与失败分支提示。 |
| 2 | 🎁 活动优惠 | 已完成 | 已补齐活动状态 API（`GET /api/v1/user/promotion/status`）与前端展示（Dashboard `PromotionBanner`），支付成功时自动计算并赠送额度。 |
| 3 | 👥 邀请返利 | 已完成 | 已补齐注册邀请码接入（`invite_code`）、用户端 API（`/api/v1/user/referral/*`）与前端邀请页（`/referral`），支付成功时按规则自动发放返利。 |
...
|10 | 🖥️ 前端界面改造 | 部分完成 | 已按 `docs/migrate-crs/zhifu.png`、`docs/migrate-crs/tanchuang.png` 补齐 `/payment` 样式与交互，并支持二维码展示/支付状态轮询，同时新增 `/payment/result` 结果页；后续仍可补强更完整的支付闭环提示（失败/过期/取消分支文案与引导）。 |
|11 | 🤖 钉钉机器人 | 已完成 | 已加入 `dingtalk.*` 配置与 webhook 发送逻辑：支付成功/失败/取消会推送钉钉机器人通知。 |
...
3. **活动优惠展示**：已补齐用户端查询接口与 Dashboard `PromotionBanner`，可视化活动倒计时与赠送比例。
4. **邀请返利入口**：已补齐注册 `invite_code`、用户端接口与 `/referral` 页面，支付成功后自动触发返利。
...
8. **钉钉机器人通知**：已支持通过 `dingtalk.webhook_url/secret` 配置机器人通知，当前用于支付事件推送；如需后台可视化配置可继续补充管理页。
