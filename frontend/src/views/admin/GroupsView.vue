...
const fallbackGroupOptions = computed(() => {
  return fallbackGroups.value
    .filter((group) => group.platform === createForm.platform)
...
const fallbackGroupOptionsForEdit = computed(() => {
  ...
  return fallbackGroups.value
    .filter((group) => group.platform === editForm.platform && group.id !== currentGroupId)
    .map((group) => ({ value: group.id, label: group.name }))
})
...
const loadFallbackGroups = async () => {
  try {
    fallbackGroups.value = await adminAPI.groups.getAll()
  } catch {
