...
const formatSystemLogDetail = (row: OpsSystemLog) => {
  ...
  if (statusCode) accessParts.push(`${t('admin.ops.systemLog.detailLabels.status')}: ${statusCode}`)
  if (latencyMs) accessParts.push(`${t('admin.ops.systemLog.detailLabels.latency')}: ${latencyMs} ms`)
  if (method) accessParts.push(`${t('admin.ops.systemLog.detailLabels.method')}: ${method}`)
  if (path) accessParts.push(`${t('admin.ops.systemLog.detailLabels.path')}: ${path}`)
  if (clientIP) accessParts.push(`${t('admin.ops.systemLog.detailLabels.ip')}: ${clientIP}`)
  if (protocol) accessParts.push(`${t('admin.ops.systemLog.detailLabels.protocol')}: ${protocol}`)
  if (accessParts.length > 0) parts.push(accessParts.join(' '))

  const corrParts: string[] = []
  if (row.request_id) corrParts.push(`${t('admin.ops.systemLog.detailLabels.requestId')}: ${row.request_id}`)
  if (row.client_request_id) {
    corrParts.push(`${t('admin.ops.systemLog.detailLabels.clientRequestId')}: ${row.client_request_id}`)
  }
  if (row.user_id != null) corrParts.push(`${t('admin.ops.systemLog.detailLabels.userId')}: ${row.user_id}`)
  if (row.account_id != null) {
    corrParts.push(`${t('admin.ops.systemLog.detailLabels.accountId')}: ${row.account_id}`)
  }
  if (row.platform) corrParts.push(`${t('admin.ops.systemLog.detailLabels.platform')}: ${row.platform}`)
  if (row.model) corrParts.push(`${t('admin.ops.systemLog.detailLabels.model')}: ${row.model}`)
  if (corrParts.length > 0) parts.push(corrParts.join(' '))

  const errors = getExtraString(extra, 'errors')
  if (errors) parts.push(`${t('admin.ops.systemLog.detailLabels.errors')}: ${errors}`)
  const err = getExtraString(extra, 'err') || getExtraString(extra, 'error')
  if (err) parts.push(`${t('admin.ops.systemLog.detailLabels.error')}: ${err}`)
...
const fetchLogs = async () => {
  ...
  try {
    ...
    appStore.showError(err?.response?.data?.detail || t('admin.ops.systemLog.fetchFailed'))
  } finally {
    loading.value = false
  }
}
...
const saveRuntimeConfig = async () => {
  ...
  try {
    ...
    appStore.showSuccess(t('admin.ops.systemLog.saveSuccess'))
  } catch (err: any) {
...
const resetRuntimeConfig = async () => {
  const ok = window.confirm(t('admin.ops.systemLog.resetConfirm'))
  if (!ok) return
  ...
  try {
    ...
    runtimeConfig.stacktrace_level = saved.stacktrace_level
    runtimeConfig.retention_days = saved.retention_days
    appStore.showSuccess(t('admin.ops.systemLog.resetSuccess'))
    await fetchHealth()
  } catch (err: any) {
    console.error('[OpsSystemLogTable] Failed to reset runtime log config', err)
    appStore.showError(err?.response?.data?.detail || t('admin.ops.systemLog.resetFailed'))
  } finally {
    runtimeSaving.value = false
  }
}
...
const cleanupCurrentFilter = async () => {
  const ok = window.confirm(t('admin.ops.systemLog.cleanupConfirm'))
  if (!ok) return
  try {
    const payload = {
    ...
    appStore.showSuccess(t('admin.ops.systemLog.cleanupSuccess', { count: res.deleted || 0 }))
    page.value = 1
...
<template>
  <section class="rounded-2xl border border-gray-200 bg-white p-4 shadow-sm dark:border-dark-700 dark:bg-dark-900/60">
          ...
          <div class="grid gap-3 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-end">
            ...
            <div class="flex flex-wrap items-center gap-2 lg:justify-end">
              ...
              <button type="button" class="btn btn-secondary btn-sm" :disabled="runtimeSaving" @click="resetRuntimeConfig">
                {{ t('admin.ops.systemLog.resetDefaults') }}
              </button>
            </div>
          </div>
        </div>
      </div>
      <p v-if="health.last_error" class="mt-2 text-xs text-red-600 dark:text-red-400">{{ t('admin.ops.systemLog.recentWriteError') }} {{ health.last_error }}</p>
    </div>
    ...
    <div class="mb-3 flex flex-wrap gap-2">
      <button type="button" class="btn btn-primary btn-sm" @click="applyFilters">{{ t('admin.ops.systemLog.apply') }}</button>
      <button type="button" class="btn btn-secondary btn-sm" @click="resetFilters">{{ t('admin.ops.systemLog.reset') }}</button>
      <button type="button" class="btn btn-danger btn-sm" @click="cleanupCurrentFilter">{{ t('admin.ops.systemLog.cleanupByFilters') }}</button>
    ...
    <div class="overflow-hidden rounded-xl border border-gray-200 dark:border-dark-700">
      <div v-if="loading" class="px-4 py-8 text-center text-sm text-gray-500">{{ t('admin.ops.systemLog.loadingLogs') }}</div>
      <div v-else-if="!hasData" class="px-4 py-8 text-center text-sm text-gray-500">{{ t('admin.ops.systemLog.noLogs') }}</div>
        ...
        <table class="min-w-full table-fixed divide-y divide-gray-200 dark:divide-dark-700">
          <thead class="bg-gray-50 dark:bg-dark-900">
            <tr>
              ...
              <th class="w-[80px] px-3 py-2 text-left text-[11px] font-semibold text-gray-500">{{ t('admin.ops.systemLog.logLevel') }}</th>
