...
func (h *AuthHandler) Register(c *gin.Context) {
	...
	_, user, err := h.authService.RegisterWithVerification(c.Request.Context(), req.Email, req.Password, req.VerifyCode, req.InviteCode, req.PromoCode, req.InvitationCode)
	if err != nil {
		response.ErrorFrom(c, err)
		return
	}

	service.ReportBaiduOcpcEvent(req.BdVid, req.BdLandingUrl, 3)
...
func (h *AuthHandler) Login(c *gin.Context) {
	...
	service.ReportBaiduOcpcEvent(req.BdVid, req.BdLandingUrl, 5)
	h.respondWithTokenPair(c, user)
}
