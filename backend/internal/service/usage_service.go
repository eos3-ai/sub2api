...
type usageLogBillingExportRepository interface {
	ExportBillingSummary(ctx context.Context, filters usagestats.UsageLogFilters) ([]UsageBillingExportRow, error)
}
...
// ExportBillingSummary returns billing summary rows for CSV export without loading raw usage logs into memory.
func (s *UsageService) ExportBillingSummary(ctx context.Context, filters usagestats.UsageLogFilters) ([]UsageBillingExportRow, error) {
	exportRepo, ok := s.usageRepo.(usageLogBillingExportRepository)
	if !ok {
		return nil, fmt.Errorf("usage log repository does not support billing export")
	}
	rows, err := exportRepo.ExportBillingSummary(ctx, filters)
	if err != nil {
		return nil, fmt.Errorf("export billing summary: %w", err)
	}
	return rows, nil
}
