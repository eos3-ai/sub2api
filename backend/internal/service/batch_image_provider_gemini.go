...
type geminiPart struct {
	...
	InlineData *geminiBatchInlineData `json:"inlineData,omitempty"`
	FileData   *geminiFileData        `json:"fileData,omitempty"`
}

type geminiBatchInlineData struct {
	MimeType string `json:"mimeType"`
...
func batchImageGeminiParts(prompt string, refs []BatchImageReference) ([]geminiPart, error) {
	...
	for _, ref := range refs {
		...
		case len(ref.Data) > 0 && fileURI == "":
			parts = append(parts, geminiPart{InlineData: &geminiBatchInlineData{
				MimeType: mimeType,
