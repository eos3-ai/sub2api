...
func parseLegacyPaymentOrderID(orderID string, lookupErr error) (int64, bool) {
	...
	if !strings.HasPrefix(orderID, defaultPaymentOrderPrefix) {
		return 0, false
	}
	trimmed := strings.TrimPrefix(orderID, defaultPaymentOrderPrefix)
...
func (s *PaymentService) doBalance(ctx context.Context, o *dbent.PaymentOrder, lease *paymentFulfillmentLease) error {
	...
	redeemCtx := ContextSkipInternalBalanceOrder(ContextSkipRedeemAffiliate(ctx))
