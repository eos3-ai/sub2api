	...
	if balance > 0 {
		...
		if s.redeemCodeRepo != nil {
			...
			if err != nil {
				...
				if err := s.redeemCodeRepo.Create(ctx, adjustmentRecord); err != nil {
					logger.LegacyPrintf("service.admin", "failed to create admin initial balance history: user_id=%d amount=%.8f err=%v", user.ID, balance, err)
				}
			}
		}
		if s.internalOrderRecorder != nil {
			...
			if _, err := s.internalOrderRecorder.RecordInternalBalanceOrder(ctx, InternalBalanceOrderInput{
				...
				logger.LegacyPrintf("service.admin", "failed to create admin initial balance order: user_id=%d amount=%.8f err=%v", user.ID, balance, err)
			}
		}
	}
	return user, nil
