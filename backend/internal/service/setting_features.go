...
// GetInvoiceDefaultItemName returns the configured invoice item label.
func (s *SettingService) GetInvoiceDefaultItemName(ctx context.Context) string {
	if s == nil || s.settingRepo == nil {
		return ""
	}
	v, err := s.settingRepo.GetValue(ctx, SettingKeyInvoiceDefaultItemName)
	if err != nil {
		return ""
	}
...
// GetAdminAPIKeyReadOnly returns the dedicated read-only administrator key.
...
// GenerateAdminAPIKeyReadOnly generates a new dedicated read-only administrator key.
...
// GetAdminAPIKeyReadOnlyStatus returns a masked read-only administrator key and
...
// DeleteAdminAPIKeyReadOnly removes the dedicated read-only administrator key.
