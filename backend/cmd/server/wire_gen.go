...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	internalBalanceOrderRecorder := service.NewInternalBalanceOrderRecorder(client, userRepository)
	...
	promoService.SetInternalBalanceOrderRecorder(internalBalanceOrderRecorder)
	...
	affiliateService.SetInternalBalanceOrderRecorder(internalBalanceOrderRecorder)
	authService := service.ProvideAuthService(client, userRepository, redeemCodeRepository, refreshTokenCache, configConfig, settingService, emailService, turnstileService, tencentCaptchaService, aliyunCaptchaService, emailQueueService, promoService, subscriptionService, affiliateService, serviceUserPlatformQuotaRepository, internalBalanceOrderRecorder)
	...
	redeemService.SetInternalBalanceOrderRecorder(internalBalanceOrderRecorder)
	...
	if recorderSetter, ok := adminService.(interface {
		SetInternalBalanceOrderRecorder(service.InternalBalanceOrderRecorder)
	}); ok {
		recorderSetter.SetInternalBalanceOrderRecorder(internalBalanceOrderRecorder)
	}
	...
	invoiceHandler := admin.NewInvoiceHandler(invoiceService, userRepository)
	...
	handlers := handler.ProvideHandlers(authHandler, userHandler, apiKeyHandler, usageHandler, redeemHandler, subscriptionHandler, announcementHandler, channelMonitorUserHandler, channelMonitorV2Handler, adminHandlers, gatewayHandler, openAIGatewayHandler, handlerSettingHandler, totpHandler, passkeyHandler, handlerPaymentHandler, paymentWebhookHandler, handlerInvoiceHandler, availableChannelHandler, modelPlazaHandler, asyncImageHandler, batchImageHandler, idempotencyCoordinator, idempotencyCleanupService, openAIQuotaAutoResetService)
